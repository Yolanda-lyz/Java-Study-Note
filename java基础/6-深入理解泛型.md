...
泛型是jdk 5.0引入的新特性，它类似于C++的模板类，意味着编写的代码可以被不同类型的对象所重写。
...
stor.setValue(134);
int value = (int) stor.getValue();
stor.setValue("hello");
String value1 = (String) stor.getValue();
```
...
```java
Storage<Integer> stor1 = new Storage<Integer>();
stor1.setValue(134);
int value =  stor1.getValue();

Storage<String> stor2 = new Storage<String>();
stor2.setValue("hello");
String value1 = (String) stor2.getValue();
```
...
从编译后的字节码可以看出，属性值value的类型被替换成Object：

![image-20201214190414712](C:\Users\11104512\AppData\Roaming\Typora\typora-user-images\image-20201214190414712.png)
...
所以，泛型实际上也是java的一种语法糖，它经泛型擦除后，实际上与我们泛型出现前定义一个通用的类的方法是一致的，也是通过一个原始可替代的类来替换。
...
类型检查是针对引用的，谁是一个引用，用这个引用调用泛型方法，就会针对这个引用调用的方法进行类型检查。
...
get方法，返回的是Object对象，有无强制类型转换？如何确定要转换的类型？

#### 5.4 类型擦除与多态冲突


#### 5.5 运行时类型检查异常--instanceof
